...
from TransformerModules import Transformer, LayerNorm, SiLU
...
class VIB1(nn.Module):
    def __init__(self, in_feat):
        super(VIB1, self).__init__()
        self.in_feat = in_feat
        self.classifier = nn.Sequential(
            nn.Linear(self.in_feat, in_feat*2),
            nn.LayerNorm(in_feat*2),
            nn.ReLU(),
            nn.Linear(in_feat*2, in_feat*2),
            nn.LayerNorm(in_feat*2),
            nn.ReLU(),
...
class TransEncoder(nn.Module):
    ...
    def forward(self, cas_emb, x_t, t, mask):
        ...
        # s_u_emb = cas_emb[:,0,:].unsqueeze(1).repeat(1, cas_emb.size(1), 1)
...
class GaussianDiffusion(nn.Module):
    def __init__(self, args, reverse):
        ...
        self.steps = args.steps  # diffusion max step
        self.beta_start = args.beta_start # min beta
        self.beta_end = args.beta_end # max beta
        ...
        # noise_scale = 0.1
        # self.betas *= noise_scale 
        # through betas to compute alphas
    ...
    def get_betas(self):
        # through beta schedules to get betas
    ...
    def q_sample(self, x_start, t, noise=None, mask=None):
        # noise:(bsz, max_length-1, dim)
        if noise is None:
            noise = torch.randn_like(x_start)

        assert noise.shape == x_start.shape
        # generate x_t based on x_0 (x_start) with re-parameter trick
        x_t = (
        ...
        if mask is None:
            ...
            # replace the output_target_seq embedding (x_0) as x_t
    ...
    def p_mean_variance(self, cas_emb, x_t, t, mask):
        ...
        # x_0 = self._predict_x_start_from_eps(x_t, t, model_output)  ## eps predict

        model_log_variance = np.log(np.append(self.posterior_variance[1], self.betas[1:]))
        # t: a time step    x_t:(bsz,seq_length,dim)
        model_log_variance = self._extract_into_tensor(model_log_variance, t, x_t.shape)

        # x_start: candidate item embedding, x_t: input_seq_embedding + out_seq_noise, output x_(t-1) distribution
        model_mean = self.q_posterior_mean_variance(x_start=x_0, x_t=x_t, t=t)
        return model_mean, model_log_variance

    def p_sample(self, cas_emb, noise_x_t, t, mask):
        #  cas_emb:input emb     noise_x_t:random sample     t:time steps
        ...
        # sample x_{t-1} from the \mu(x_{t-1}) distribution based on the reparameter trick
    ...
    def reverse_p_sample(self, cas_emb, noise_x_t, mask):
        # cas_emb:(bsz, max_length-1, dim) noise_x_t:(bsz, max_length-1, dim) mask:(bsz, max_length-1)
        device = next(self.Encoder.parameters()).device
        indices = list(range(self.steps))[::-1]

        # from T to 0, reversion iteration
        for i in indices:
...
class DucDiff(nn.Module):
    def __init__(self, args):
        ...
        self.ib1 = VIB1(args.hidden_size)
        # self.ib_future = VIB(in_feat=args.hidden_size, out_feat= args.compress_emb,num_class=args.user_size)
        ...
        # time encoder
        # self.time_encoder = TimeEncoder(args.hidden_size)
        ...
        # diffurec optimizer
        # self.optimizerAdam = torch.optim.Adam(self.parameters(), lr=args.lr, weight_decay=args.weight_decay)
        # self.lr_scheduler = optim.lr_scheduler.StepLR(self.optimizerAdam, step_size=args.decay_step, gamma=args.gamma)
    ...
    def forward(self, cas_seq, label):
        ...
        # get user embedding of past and future
        ...
        past_output_ib = self.ib1(past_hidden)
        future_output_ib = self.ib1(future_hidden) # [batch,seq,user_size]
        ...
        past_output_ib = torch.matmul(past_output_ib, self.user_embeddings.weight.t())
        future_output_ib = torch.matmul(future_output_ib, self.user_embeddings.weight.t())
        ...
        future_mask = get_previous_user_mask(label.cuda(), self.user_size)
        future_output_noib_mask = (future_output_ib + mask).view(-1, past_output_noib.size(-1))
        # future_output_noib = (future_output_noib + mask).view(-1, future_output_noib.size(-1))
        # past_output_ib = (past_output_ib + mask).view(-1, past_output_ib.size(-1))
        # future_output_ib = (future_output_ib + mask).view(-1, future_output_ib.size(-1))
        # output_past = (past_output + mask).view(-1, past_output.size(-1))
        # future_output = (future_output + mask).view(-1, future_output.size(-1))

        return past_output_noib_mask, future_output_noib_mask, past_output_noib.view(-1, future_output_ib.size(-1)), future_output_noib.view(-1, future_output_ib.size(-1)), \
