...
class datasets(Dataset):
    ...
    def __getitem__(self, index):
        ...
        inp_fru = inp[2:]
        nonzero_elems_fur = np.nonzero(inp_fru)[0]
        cascades_fru = np.array(inp_fru + (max_n_node - len(nonzero_elems_fur)) * [0])

        cascades = np.array(inp + (max_n_node - len(nonzero_elems)) * [0])
        cascades_time = np.array(inp_t + (max_n_node - len(nonzero_elems)) * [inp_t[-1]])
